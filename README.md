...
|---|---|---|
| PRD agreed | Big edits before a PRD (default: >2 files / >40 lines), heredocs counted by their lines, shell writes that mix the PRD with other files | Question quality; shell writes are matched by pattern, so an exotic way to spell a redirect gets through |
...
```bash
python3 hooks/tests/gates.test.py      # 76 gate cases — both violations and false blocks
