...
플러그인은 한 번 깔면 전역이라 게이트는 이미 켜져 있다. 기존 프로젝트에 필요한 건 그 프로젝트의
규칙을 적어 두는 것뿐이다.

```bash
cd ~/Dev/your-project
touch CLAUDE.md          # "## Conventions" 절에, 이 프로젝트가 이미 지키고 있는 규칙을 적는다
```

인터뷰가 그걸 먼저 읽고 **거기 적힌 건 묻지 않는다.** 승인 게이트는 계획을 그것과 대조한다.
...
승인과 검수는 서브에이전트가 돈다. `agents/advisor.md` 의 `model:` 은 가장 능력 있는 모델로 두는데,
**일부러 작업하는 세션과 다른 모델**이다 — 실행자와 같은 맹점을 가진 검수자는 두 번째 의견이 아니다.
비용은 작다. 작업당 두 번, 대화가 아니라 계획과 코드만 읽는다.

**그 모델에 접근이 없으면 그 한 줄을 바꾸면 된다**(`opus` 로도 된다). advisor 가 못 뜨면 승인도 검수도
오지 않고, 게이트 둘이 이유도 모르게 닫힌 채로 있는다. `/midnight-vibe:doctor` 가 실제로 불러서 확인한다.
